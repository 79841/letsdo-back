...
async def get(chatroomId:int, db: Session=Depends(get_db)):
    messages = db.query(Message).filter(Message.chatroom_id == chatroomId).all()
    messages = [{"content":message.content, "userId":message.user_id, "timestamp":message.timestamp, "messageFrom":message.user.username, "chatroomId":message.chatroom.id} for message in messages]
    return messages
        ...
        while True:
            ...
            for participant in [messageTo, current_user.id]:
                if participant in connectedClients:
                    message = {"content":content, "userId":current_user.id, "messageFrom":current_user.username, "chatroomId":chatroom.id, "timestamp":message.timestamp.strftime('%Y-%m-%d %H:%M:%S')}
